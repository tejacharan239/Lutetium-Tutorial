...
def A(content, cls):
    return '<g class="%s"><g class="bl">%s</g></g>' % (cls, content)

def L(content, cls, style=''):
...
def frame(uid, body, title):
    ...
    BX, BY = 200, 90
    return ('%s<rect x="%d" y="%d" width="%d" height="%d" fill="%s"/><g class="bl">%s</g>%s%s'
            % (defs(uid), -BX, -BY, W + 2 * BX, H + 2 * BY, P['paper'], body,
               screen(W + 2 * BX, H + 2 * BY, uid, 0.34, 0.09, 0.20, -BX, -BY),
               heading(26, 34, title)))
...
def lightcone(x, y, z, o, dx, dy, spread=34, colour=None):
    ...
    return ('<g class="flicker"><polygon points="%.1f,%.1f %.1f,%.1f %.1f,%.1f" fill="%s" opacity=".34"/></g>'
...
def s01():
    ...
    s += A(plate(20, 236, 258, 128, 'PRRT',
                 ['Lu-177 DOTATATE', 'grade 1 . stage 4', 'neuroendocrine tumour', '', 'thirteen scenes'], P['gold']), 'a a-stamp d2')
...
def s02():
    ...
    for i, (dx, dy, r) in enumerate([(-86, -58, 12), (-50, -64, 9), (-34, -20, 14), (-76, -12, 8), (-58, -38, 7)]):
        s += A(cylinder(dx, dy, r, 48, 5, P['plum'], P['plumD'], O)
               + dome(dx, dy, r, 53, r * .8, P['plumL'], O, rim=P['plumD']), 'a a-drop d%d' % (5 + i))
    ...
    s += A(plate(438, 44, 258, 82, 'GRADE 1', ['Ki-67 <= 2%', 'well differentiated'], P['teal']), 'a a-stamp d2')
    s += A(plate(438, 140, 258, 82, 'STAGE 4', ['distant metastases', 'liver in ~85%'], P['plum']), 'a a-stamp d4')
    s += A(plate(24, 300, 300, 66, 'THE BIND', ['too slow for chemotherapy,', 'too scattered for surgery'], P['rust']), 'a a-stamp d11')
...
def s03():
    ...
    s += A(plate(24, 302, 232, 82, 'NORMAL CELL', ['a few receptors', 'baseline SSTR2'], P['teal']), 'a a-stamp d7')
    s += A(plate(452, 46, 244, 82, 'GRADE 1 NET CELL', ['SSTR2 over-expressed', '10-100x more receptor'], P['plum']), 'a a-stamp d4')
...
def s04():
    ...
    s += A(cage, 'a a-up d3')
    s += A(L(cylinder(4, -26, 13, 20, 12, P['gold'], P['goldD'], O) + dome(4, -26, 13, 32, 15, P['goldL'], O, rim=P['goldD']), 'pulse')
           + ring(4, -26, 19, 30, P['goldL'], 1.8, O, ' opacity=".7"'), 'a a-drop d5')
    ...
    s += A(plate(20, 292, 206, 82, 'OCTREOTATE', ['finds the receptor', 'the address'], P['teal']), 'a a-stamp d2')
    s += A(plate(240, 316, 190, 66, 'DOTA', ['cages the metal'], P['gold']), 'a a-stamp d4')
    s += A(plate(470, 40, 226, 100, 'LUTETIUM-177', ['beta emitter', 'half-life 6.65 d', 'range 0.5-2 mm'], P['rust']), 'a a-stamp d5')
...
def s05():
    ...
    for i, (lx, ly, r) in enumerate([(64, -44, 16), (124, -72, 12), (94, -8, 19), (156, -34, 10)]):
        s += A(cylinder(lx, ly, r, 0, 5, P['gold'], P['goldD'], O)
               ...
               + beam(lx, ly, 7, O, P['goldL'], 4, 13, -20), 'a a-drop d%d' % (6 + i))
    s += A(figure(150, 40, 0, O, P['rust'], P['cream'], .9), 'a a-in d9')
    s += A(plate(22, 300, 250, 82, 'Ga-68 DOTATATE PET', ['same peptide,', 'imaging payload'], P['navyL']), 'a a-stamp d3')
    s += A(plate(408, 286, 288, 98, 'KRENNING 3-4 REQUIRED',
                 ['uptake above normal liver', 'dark lesion = no treatment'], P['gold']), 'a a-stamp d10')
...
def s06():
    ...
    for i in range(3):
        ...
        s += A(box(112 + 44 * math.cos(a) - 6, -52 + 44 * math.sin(a) - 6, 26, 12, 12, 11,
                   P['tealL'], P['tealD'], P['teal'], O), 'a a-drop d%d' % (7 + i))
    s += A(cylinder(186, -6, 9, 0, 9, P['gold'], P['goldD'], O)
           + arrow((176, -14, 12), (206, 10, 12), O, P['rust'], 2.2), 'a a-in d10')
    s += A(plate(20, 292, 262, 90, 'AMINO ACIDS FIRST', ['lysine + arginine, 2.5%', 'block the kidney uptake'], P['teal']), 'a a-stamp d4')
    s += A(plate(454, 44, 242, 90, 'THEN THE DOSE', ['7.4 GBq over 30 min', 'four cycles, q8 weeks'], P['gold']), 'a a-stamp d5')
    s += A(plate(454, 296, 242, 68, 'OCTREOTIDE HELD', ['4 weeks before'], P['rust']), 'a a-stamp d11')
...
def s07():
    ...
    for i in range(10):
        a = 2 * math.pi * i / 10 + .3
        s += A(mast(10 + 76 * math.cos(a), -20 + 76 * math.sin(a), 44, 13, O, P['gold']), 'a a-drop d2')
    s += A(cylinder(10, -20, 24, 0, 18, P['navyXL'], P['navyL'], O)
           + dome(10, -20, 24, 18, 18, P['navyL'], O, rim=P['navy'])
           + L(disc(10, -20, 11, P['gold'], 37, O), 'pulse')
           + L(ring(10, -20, 17, 37, P['goldL'], 1.8, O, ' opacity=".75"'), 'ripple'), 'a a-drop d8')
    wx0, wy0 = pt(-70, 104, 0, O)
    ...
    s += A(plate(20, 300, 214, 82, '2 . BIND SSTR2', ['the gate opens', 'Kd < 1 nmol/L'], P['teal']), 'a a-stamp d5')
    s += A(plate(482, 52, 214, 82, '3 . SWALLOWED', ['pulled inside', 'by endocytosis'], P['plum']), 'a a-stamp d7')
    s += A(plate(482, 296, 214, 88, '4 . STRANDED', ['peptide digested,', 'the metal stays'], P['rust']), 'a a-stamp d9')
...
def s08():
    ...
    for i in range(7):
        a = 2 * math.pi * i / 7 + .2
        s += A(L(arrow((-84 + 34 * math.cos(a), -20 + 34 * math.sin(a), 30),
                       (-84 + 80 * math.cos(a), -20 + 80 * math.sin(a), 30), O, P['goldL'], 1.8),
                 'flow', '--fd:-%.2fs' % (i * 0.2)), 'a a-in d%d' % (3 + i % 4))
    for i, (wx, wy) in enumerate([(-10, -84), (18, -48), (-28, -112)]):
        s += A(L(disc(wx, wy, 11, P['teal'], 6, O) + disc(wx + 22, wy + 12, 9, P['rust'], 6, O), 'jit',
                 'animation-delay:-%.2fs' % (i * .17)), 'a a-drop d%d' % (6 + i))
    hx, hy = 112, -46
    rails, rungs = '', ''
    ...
    s += A(plate(20, 292, 250, 90, 'ONE ELECTRON PER DECAY', ['mean 134 keV', 'splits water into radicals'], P['gold']), 'a a-stamp d5')
    s += A(plate(446, 268, 250, 108, 'DOUBLE-STRAND BREAK',
                 ['both rails cut at once', 'no template left to copy', 'repair is outrun'], P['rust']), 'a a-stamp d12')
...
def s09():
    ...
    s += A(ring(-36, -18, 118, 1.5, P['goldL'], 2.6, O, ' stroke-dasharray="7 6" opacity=".9"')
           + L(ring(-36, -18, 118, 1.5, P['gold'], 3.2, O, ' opacity=".9"'), 'wave'), 'a a-in d1')
    s += A(cell(-36, -18, 34, O, P['plum'], P['plumD'], 6, P['gold'], 15, core=P['gold'], glow=True), 'a a-drop d2')
    s += A(cell(44, -66, 30, O, P['plumL'], P['plumD'], 5, P['gold'], 14, core=P['gold']), 'a a-drop d3')
    for i, (cx, cy) in enumerate([(-116, -58), (-4, -110), (40, 34), (-96, 54), (-130, -132)]):
        s += A(cell(cx, cy, 27, O, P['navyXL'], P['navyL'], 0, None, 12), 'a a-drop d%d' % (4 + i))
    s += A(''.join(L(arrow((-36, -18, 24), (-36 + 104 * math.cos(2 * math.pi * i / 6), -18 + 104 * math.sin(2 * math.pi * i / 6), 24), O, P['goldL'], 1.6),
                     'flow', '--fd:-%.2fs' % (i * .23)) for i in range(6)), 'a a-in d6')
    s += A(line((-154, 122, 1), (82, 122, 1), P['goldL'], 2.6, O)
           + mast(-154, 122, 1, 8, O, P['goldL'], 2.0) + mast(82, 122, 1, 8, O, P['goldL'], 2.0), 'a a-up d9')
    s += A(plate(20, 296, 250, 90, 'BETA RANGE 0.5-2 mm', ['a cell is ~20 um across', 'so 25-100 cells deep'], P['gold']), 'a a-stamp d9')
    s += A(plate(446, 40, 250, 124, 'DOSE ALSO LANDS HERE',
                 ['kidneys . creatinine', 'bone marrow . blood count', 'spleen and liver', 'these cap the total dose'], P['rust']), 'a a-stamp d10')
    s += A(plate(446, 286, 250, 90, 'CROSSFIRE', ['neighbours with no receptor', 'are irradiated anyway'], P['plum']), 'a a-stamp d11')
...
def s10():
    ...
    for i in range(4):
        ...
        s += A(cylinder(x, y, 30, 0, 16, P['navyXL'], P['navyL'], O)
               ...
               + figure(x + 26, y + 26, 0, O, P['rust'], P['cream'], .85), 'a a-drop d%d' % (1 + i * 2))
        if i < 3:
            s += A(arrow((x + 34, y - 34, 2), (x + 58, y - 58, 2), O, P['paper'], 2.0), 'a a-in d%d' % (2 + i * 2))
    DL = [.3, .9, 1.55, 2.25, 2.95, 3.7, 4.35]
    for i in range(4):
    ...
    s += A(plate(20, 296, 268, 90, 'SPECT/CT AT 24 HOURS', ['on the 208 keV gamma', 'confirms where dose landed'], P['teal']), 'a a-stamp d10')
...
def s11():
    ...
    body, tag = tower(-58, 58, 65.2, P['gold'], P['goldD'], .3)
    s += A(body, 'd1') + A(tag, 'a a-stamp d1')
    body, tag = tower(76, -76, 10.8, P['navyXL'], P['navyL'], .9)
    s += A(body, 'd3') + A(tag, 'a a-stamp d3')
    s += A(figure(-6, 6, 0, O, P['rust'], P['cream'], .95)
           + figure(14, 26, 0, O, P['teal'], P['cream'], .95), 'a a-in d5')
    s += A(plate(18, 54, 236, 70, 'Lu-177 DOTATATE', ['progression-free, month 20'], P['gold']), 'a a-stamp d2')
    s += A(plate(470, 54, 226, 70, 'OCTREOTIDE 60 mg', ['same endpoint'], P['navyL']), 'a a-stamp d4')
    s += A(plate(20, 300, 210, 84, 'HAZARD RATIO', ['0.21 (0.13-0.33)'], P['gold']), 'a a-stamp d6')
    s += A(plate(244, 300, 210, 84, 'RESPONSE', ['18% vs 3% shrank'], P['teal']), 'a a-stamp d7')
    s += A(plate(468, 300, 228, 84, 'SURVIVAL', ['48.0 vs 36.3 months', 'not significant'], P['rust']), 'a a-stamp d8')
...
def s12():
    ...
    s += A(plate(20, 292, 258, 90, 'EARLY . WEEKS', ['fatigue, nausea', 'counts dip'], P['teal']), 'a a-stamp d10')
    s += A(plate(292, 292, 194, 90, 'LATE . YEARS', ['~2% MDS or AML'], P['rust']), 'a a-stamp d11')
    s += A(plate(500, 292, 196, 90, 'SUCCESS', ['disease held still'], P['gold']), 'a a-stamp d12')
...
def s13():
    ...
    s += A(plate(96, 58, 528, 148, 'EDUCATIONAL MATERIAL',
                 ['This explains a mechanism. It is not medical advice, not a',
                  ...
                  'Lutathera prescribing information . WHO 2022 classification'], P['gold'], fs=11), 'a a-stamp d2')
